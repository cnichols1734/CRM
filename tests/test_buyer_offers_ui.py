...
def test_blank_offer_form_clears_title_policy_payer(app, seed, owner_a_client):
    with app.app_context():
        _cleanup_offers(seed['org_a'], seed['tx_a'])
        offer = _add_offer(
            seed['org_a'],
            seed['owner_a'],
            seed['tx_a'],
            buyer_names='Clear Title Policy Buyer',
        )
    ...
    response = owner_a_client.post(
        f'/transactions/{seed["tx_a"]}/offers/{offer_id}',
        json={'terms_data': {'title_policy_payer': ''}},
    )
    assert response.status_code == 200, response.get_data(as_text=True)
    ...
    try:
        with app.app_context():
            saved = db.session.get(SellerOffer, offer_id)
            stored = db.session.get(SellerOfferVersion, version_id)
            assert saved.title_policy_payer is None
            assert 'title_policy_payer' not in (saved.terms_summary or {})
            assert 'title_policy_payer' not in (stored.terms_data or {})
            from services.offer_summary_email import resolved_title_policy_payer
            assert resolved_title_policy_payer(saved) is None
    finally:
        with app.app_context():
            _cleanup_offers(seed['org_a'], seed['tx_a'])
...
def test_offer_form_js_sends_a_blank_non_realty_field():
    ...
    assert "field === 'title_policy_payer'" in source
    assert 'field === \'non_realty_items\' || field === \'title_policy_payer\'' in source
