...
| Daily Health Check Cron | `0 2,14 * * *` | `python -m jobs.daily_health_check` (9:00 AM + 9:00 PM CT digest) |
| Health Alert Cron | `*/5 * * * *` | `python -m jobs.daily_health_check --alert-only` (email only on WARN/FAIL + recovery; 30m cooldown) |
