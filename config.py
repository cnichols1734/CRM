...
class Config:
    ...
    # because the client hired the brokerage. The address only has to be a
    # sender SendGrid will accept; replies are addressed to the agent.
    CLIENT_EMAIL_FROM_EMAIL = os.getenv(
        'CLIENT_EMAIL_FROM_EMAIL', 'info@origenrealty.com'
    )
    CLIENT_EMAIL_FROM_NAME = os.getenv('CLIENT_EMAIL_FROM_NAME', 'Origen Realty')
    # Brand marks for that same mail. Both sit on the slate masthead and footer
    # bands, so a replacement has to read on #6C7F93. An organization with its
    # own logo_url overrides the mark; the wordmark then drops to text.
    #
    # These point at the public Supabase bucket rather than /static because an
    # image in a delivered email has to keep loading regardless of deploys, and
    # a local preview would otherwise ask a client's mail app for localhost.
    # Generated by scripts/split_brand_logo.py, published by
    # scripts/upload_brand_assets.py.
    CLIENT_EMAIL_BRAND_MARK = os.getenv(
        'CLIENT_EMAIL_BRAND_MARK',
        'https://ikdcpkuwpaejbgyapzhr.supabase.co/storage/v1/object/public/'
        'brand-assets/origen/email/origen-og-mark-slate.png',
    )
    CLIENT_EMAIL_BRAND_WORDMARK = os.getenv(
        'CLIENT_EMAIL_BRAND_WORDMARK',
        'https://ikdcpkuwpaejbgyapzhr.supabase.co/storage/v1/object/public/'
        'brand-assets/origen/email/origen-wordmark-slate.png',
    )
