...
def _paragraph(text: str) -> str:
    ...
    for chunk in chunks:
        ...
        out.append(
            f'<p style="margin:0 0 18px 0;font-family:{FONT};font-size:16px;'
...
def _heading(block: dict) -> str:
    size = 26 if block.get('level', 'h2') == 'h2' else 19
    tag = block.get('level', 'h2')
    return (
        f'<{tag} style="margin:0 0 18px 0;font-family:{FONT};font-size:{size}px;'
        f'font-weight:700;color:{INK};line-height:1.3;">{esc(block["text"])}</{tag}>'
    )


def _bullets(block: dict) -> str:
    items = '\n'.join(
        f'<li style="margin:0 0 8px 0;">{esc(item)}</li>'
        for item in block.get('items', [])
    )
    return (
        f'<ul style="margin:0 0 20px 0;padding-left:22px;font-family:{FONT};'
        f'font-size:16px;color:{INK_BODY};line-height:1.6;">{items}</ul>'
    )
...
def _button(block: dict) -> str:
    ...
    <tr><td align="center" style="padding:6px 0 30px 0;">
        <a href="{esc(block["url"])}" style="display:inline-block;font-family:{FONT};background-color:{ACCENT};background:linear-gradient(135deg, {ACCENT} 0%, {ACCENT_DARK} 100%);color:#ffffff;font-size:15px;font-weight:600;text-decoration:none;padding:14px 40px;border-radius:10px;">{esc(block["label"])}</a>
    </td></tr>
</table>'''
...
def _image(block: dict) -> str:
    ...
    if block.get('caption'):
        caption = (
            ...
            f'color:{INK_FAINT};text-align:center;">{esc(block["caption"])}</p>'
        )
...
def _listing_card(block: dict) -> str:
    ...
    if block.get('caption'):
        rows.append(
            f'<p style="margin:0 0 6px 0;font-family:{FONT};font-size:13px;'
            f'color:{INK_BODY};line-height:1.6;">{esc(block["caption"])}</p>'
        )
    if block.get('url'):
        rows.append(
...
def _quote(block: dict) -> str:
    ...
    if block.get('attribution'):
        attribution = (
            ...
            f'font-weight:600;color:{INK_MUTED};">&mdash; {esc(block["attribution"])}</p>'
        )
    ...
    <tr><td style="background-color:{INSET_BG};border-left:4px solid {ACCENT};border-radius:10px;padding:18px 22px;">
        <p style="margin:0;font-family:{FONT};font-size:16px;color:{INK};line-height:1.65;font-style:italic;">{esc(block["text"])}</p>
        {attribution}
    </td></tr>
</table>'''
...
def _signature(ctx: ShellContext) -> str:
    ...
    <tr><td style="border-top:1px solid {HAIRLINE};padding:20px 0 0 0;">{body}</td></tr>
</table>'''
...
def render_blocks_html(blocks: list[dict], ctx: ShellContext) -> str:
    return '\n'.join(
        fragment
        for fragment in (render_block(b, ctx) for b in _collapse_rules(blocks))
        if fragment
    )
...
def render(blocks: list[dict], ctx: ShellContext, *, validate: bool = True) -> RenderedEmail:
    ...
    body_html = wrap(render_blocks_html(blocks, ctx), ctx)
...
def preview(blocks: list[dict], ctx: ShellContext, subject: str) -> tuple[str, str]:
    """Render with example data, for the studio preview pane."""
    rendered = render(blocks, ctx)
    filled_subject, filled_html, _, _ = personalize(
        rendered, subject, mf.sample_values(),
    )
